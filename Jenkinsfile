pipeline {
  ...
  environment {
    ...
    DOCKER_HUB_ACCOUNT = 'johnharris85'
  }
  stages {
      ...
      steps {
        container('docker') {
            dir ("site") {
                ...
                sh ("docker tag ${DOCKER_HUB_ACCOUNT}/${DOCKER_IMAGE_NAME}:${env.BUILD_NUMBER} ${DOCKER_HUB_ACCOUNT}/${DOCKER_IMAGE_NAME}:${GIT_BRANCH,fullName=false}-latest")
